...
const ISSUE_CERTIFICATE_TARGET = `${PACKAGE_ID}::${MODULE_NAME}::issue_certificate` as const;

export type MintCertificateInput = {
  signAndExecuteTransactionBlock: any; // Use 'any' to match useSignAndExecuteTransaction from dapp-kit
  recipientWallet: string;
  certificateId: string; // Unique certificate ID (e.g., requestId)
  title: string;
  description: string;
  imageUrl: string;
  issueDate: number; // Unix timestamp in milliseconds
};
...
export const mintCertificate = async ({
  ...
  const extractObjectId = (data: any, source: string): string | undefined => {
  ...
  if (response.objectChanges && Array.isArray(response.objectChanges)) {
    ...
    const createdObjects = response.objectChanges.filter(
      (change: any) => change.type === 'created' || change.type === 'publish'
    );
    
    if (createdObjects.length > 0) {
      // Try to find Certificate object by type
      const CERT_TYPE = `${PACKAGE_ID}::${MODULE_NAME}::Certificate`;
      const certificateObject = createdObjects.find(
        (change: any) => {
      ...
      try {
        ...
        if (txDetails.objectChanges && Array.isArray(txDetails.objectChanges)) {
          ...
          if (createdObjects.length > 0) {
            ...
            objectId = (certificateObject as any)?.objectId || (createdObjects[0] as any)?.objectId;
            if (objectId) {
